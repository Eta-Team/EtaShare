...
describe 'Test Collaborator Handling' do
  ...
  before do
    ...
    @another_account = EtaShare::Account.create(@another_account_data)
  ...
  describe 'Adding accessors to a link' do
    it 'HAPPY: should add a valid collaborator' do
      req_data = { email: @another_account.email }

      header 'AUTHORIZATION', auth_header(@account_data)
      put "api/v1/links/#{@link.identifier}/accessors", req_data.to_json
  ...
  describe 'Removing accessors from a link' do
    it 'HAPPY: should remove with proper authorization' do
      @link.add_accessor(@another_account)
      req_data = { email: @another_account.email }

      header 'AUTHORIZATION', auth_header(@account_data)
      delete "api/v1/links/#{@link.identifier}/accessors", req_data.to_json
